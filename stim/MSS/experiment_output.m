...
if ~isempty(run_info)
    ...
    fprintf('Tag\tTrial\tOrder\tOnset\tDur\tRT\tResponse\n\n');

    for i = 1:length(run_info.onsets);
        fprintf('%s\t%d\t%d\t%.4f\t%.4f\t%.3f\t%s\n',run_info.tag{i},i,run_info.trial_order(i),run_info.onsets(i),run_info.durations(i),run_info.rt(i),run_info.responses{i});    
    end;
end;
...
if ~isempty(key_presses)
    ...
    fprintf('NO KEYS WERE PRESSED DURING THIS RUN\n');
end;
...
if PRINT_OUTPUT
    ...
    fprintf(fid,'Tag\tTrial\tOrder\tOnset\tDur\tRT\tResponse\n\n');

    if ~isempty(run_info)
        for i = 1:length(run_info.onsets);
            fprintf(fid,'%s\t%d\t%d\t%.4f\t%.4f\t%.3f\t%s\n',run_info.tag{i},i,run_info.trial_order(i),run_info.onsets(i),run_info.durations(i),run_info.rt(i),run_info.responses{i});      
        end;
    end;
