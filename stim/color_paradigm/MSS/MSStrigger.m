...
experiment_name='Smoking_Study';
experiment_code='SMK';  % this should be a short (2-3 letter) code for the experiment
                        % it will be put as part of the data file name
script_revision_date='7/21/10';
    ...
    %Revisions
        ...
        %7/21/10: updated to use trigger
        
experiment_notes='In this version of SMK, we show video-based cessation ads and collect behavioral responses (I can relate to this ad, This ad makes me more determined to quit, This ad makes me feel that I can quit)';
...
% if button_box,
%     for n=1:numDevices,
%         if (strcmp(devices(n).transport,'USB') && strcmp(devices(n).usageName,'Keyboard') && strcmp(devices(n).product,'fORP Interface')),
%             inputDevice=n;
...
for i = 1:length(st),
    ...
    if ~isempty(maxTime(i)) && st(i) ~= 's'
        ...
        if maxTime(i) == 0 %this happens if left blank in worksheet (textread function reads blanks as zeros)
            ...
            %fprintf('note: reverted to default stimulus duration for cycle %d, st = %s, maxTime(i) = %f\n',i,stFile{i},maxTime(i))
        end;
    elseif st(i) == 's'
