...
for i = 1:length(d)
    if d(i).isdir && (~strcmp(d(i).name,'.') && ~strcmp(d(i).name,'..'))
        cleaner([direc filesep  d(i).name])
    elseif regexp(d(i).name,'\.mat$','ONCE')
        fprintf('\n*********** cleaning %s\n ***********',d(i).name)
        clear run_info
        clear key_presses
        load([direc filesep d(i).name])
        if regexp(d(i).name,'^moviebackup','ONCE')
           old_key = cell(1,length(key_presses_movie2));
            for j = 1:length(key_presses_movie2)
                old_key{j} = key_presses_movie2(j).key;
            end; 
            key_presses_movie2 = clean_output(key_presses_movie2);
            
            fprintf('key_presses_movie2.key\n')
            fprintf('OLD\tNEW\n')
            for k = 1:length(old_key)
                if ~strcmp(old_key{k},key_presses_movie2(k).key)
                fprintf('%s\t%s\n',old_key{k},key_presses_movie2(k).key)
                end;
            end;
            
            save([direc filesep d(i).name],'key_presses_movie2')
        else
